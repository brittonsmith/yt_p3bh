...
def set_yticks_and_grids(my_axes, major, minor, grid=True):
    if major is not None:
        my_axes.yaxis.set_ticks(major)
    if minor is not None:
        my_axes.yaxis.set_ticks(minor, minor=True)
...
if __name__ == "__main__":
    my_fig = GridFigure(2, 2, figsize=(8, 6),
                        left_buffer=0.13, right_buffer=0.03,
                        bottom_buffer=0.10, top_buffer=0.09,
                        vertical_buffer=0, horizontal_buffer=0.13)
    ...
    for im, my_axes in enumerate(my_fig):
        my_row, my_column, my_index = my_fig._get_index(im)
        ...
        if im == 0:
            ...
            biold = np.digitize(told, tbins) - 1
            for ti in range(tbins.size):
                data.append(get_distribution(grall[bi == ti], 3))
                dataold.append(get_distribution(grold[biold == ti], 3))
            data = np.rollaxis(np.array(data), 1)
            dataold = np.rollaxis(np.array(dataold), 1)
            pdata = data[1]
            ...
            set_yticks_and_grids(my_axes,
                                 np.logspace(-20, -6, 8),
                                 np.logspace(-20, -7, 14))
            my_axes.set_ylim(1e-20, 1e-7)
        ...
        if im == 1:
            ...
            set_yticks_and_grids(my_axes,
                                 np.logspace(-29, -19, 6),
                                 np.logspace(-29, -19, 11))
            my_axes.set_ylim(1e-29, 1e-19)
        ...
        if im == 2:
            ...
            for i, p in enumerate(ptypes):
                t = ds.data[p, "time"].to("Myr")
                m = ds.data[p, "mass"].to("Msun")
                gr = ds.data[p, "sound_speed"].to("km/s")
                tall.extend(t[:-1])
                grall.extend(gr)
                my_axes.plot(t[:-1], gr, color=cmap(i/(nbh-1)), alpha=0.7)
                isf = np.where(t > t_nosf)[0][0]

            my_axes.yaxis.set_label_text("c$_{\\rm s}$ [km/s]")
        ...
        if im == 3:
            ...
            for i, p in enumerate(ptypes):
                t = ds.data[p, "time"].to("Myr")
                m = ds.data[p, "mass"].to("Msun")
                gr = ds.data[p, "relative_velocity"].to("km/s")
                tall.extend(t[:-1])
                grall.extend(gr)
                my_axes.plot(t[:-1], gr, color=cmap(i/(nbh-1)), alpha=0.7)
                isf = np.where(t > t_nosf)[0][0]

            my_axes.yaxis.set_label_text("v$_{\\rm rel}$ [km/s]")
        ...
        if my_row == 0:
            my_axes.xaxis.set_major_formatter(ticker.NullFormatter())
            my_axes.tick_params(which="both", direction="in", axis="x")
            my_axes.get_yticklabels()[0].set_visible(False)
            ...
            tx = my_axes.twiny()
            tx.set_xlim(*xlim)
            tx.xaxis.set_ticks(np.linspace(125, 500, 16), minor=True)
            tx.xaxis.set_minor_formatter(ticker.NullFormatter())
            tx.xaxis.set_major_formatter(ticker.NullFormatter())
            tx.tick_params(which="both", direction="in", axis="x")

    pyplot.savefig("figures/p2p_conditions.pdf")
